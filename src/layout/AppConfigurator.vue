...
// Load theme settings from stored values
onMounted(() => {
    loadThemeSettings();
});
...
function applyStoredThemeSettings(settings) {
    if (settings.theme_preset) {
        preset.value = settings.theme_preset;
        setPreset(settings.theme_preset);
    }

    if (settings.primary_color_name) {
        const primaryColorObj = primaryColors.value.find((c) => c.name === settings.primary_color_name);
        if (primaryColorObj) {
            setPrimary(primaryColorObj.name);
            applyTheme('primary', primaryColorObj);
        }
    }

    if (settings.surface_name) {
        const surfaceObj = surfaces.value.find((s) => s.name === settings.surface_name);
        if (surfaceObj) {
            setSurface(surfaceObj.name);
            applyTheme('surface', surfaceObj);
        }
    }
    ...
    if (settings.menu_mode) {
        ...
        setMenuMode(settings.menu_mode);
    }
}
